/* 
  ...
  This is SGLIB version 1.0.7

  (C) by Marian Vittek, Bratislava, http://www.xref-tech.com/sglib, 2003-11
...
#define SGLIB_HASH_TAB_ADD_IF_NOT_MEMBER(type, table, dim, elem, hash_function, comparator, resultIndex, member){\
  unsigned _pos_;\
  type     *_elem_;\
...
#define SGLIB_DEFINE_HASHED_TABLE_FUNCTIONS(type, dim, hash_function, comparator) \
  ...
  type *sglib_hashed_##type##_it_init_on_equal(struct sglib_hashed_##type##_iterator *it, type *table[dim], int (*subcomparator)(type *, type *), type *equalto) {\
    ...
    it->equalto = equalto;\
    for(_i_=0; _i_<(dim) && table[_i_]==NULL; _i_++) ;\
    it->currentIndex = _i_;\
    if (_i_<(dim)) return(table[_i_]);\
    return(NULL);\
  }\
  ...
  type *sglib_hashed_##type##_it_current(struct sglib_hashed_##type##_iterator *it) {\
    return(it->table[it->currentIndex]);\
  }\
  type *sglib_hashed_##type##_it_next(struct sglib_hashed_##type##_iterator *it) {\
    int _i_;\
    _i_=it->currentIndex;\
    if (_i_<(dim)) {\
      for(_i_++; _i_<(dim) && it->table[_i_]==NULL; _i_++) ;\
    }\
    it->currentIndex = _i_;\
    if (_i_<(dim)) return(it->table[_i_]);\
    return(NULL);\
  }
